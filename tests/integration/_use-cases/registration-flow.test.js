...
describe("Use case: Registration Flow (all successful)", () => {
  ...
  test("Create user account", async () => {
    const createUserResponse = await fetch(`${webserver.origin}/api/v1/users`, {
      ...
      body: JSON.stringify({
        ...
        email: "registration.flow@menuspoiler.com.br",
  ...
  test("Receive activation email", async () => {
    ...
    expect(lastEmail.sender).toBe("<contato@menuspoiler.com.br>");
    expect(lastEmail.recipients[0]).toBe(
      "<registration.flow@menuspoiler.com.br>",
    );
    expect(lastEmail.subject).toBe("Ative seu cadastro no Menu Spoiler!");
